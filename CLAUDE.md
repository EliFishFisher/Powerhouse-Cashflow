...
| 2026-04-16 | **Transactions page filters** — Added direction (In/Out/All), currency, date range, amount range filters. Dynamic entity options built from actual data (includes subsidiaries). `ToggleGroup` helper component. Stats use `t.netUSD` for cross-currency accuracy. "▼ More" expandable row for date+amount filters. "✕ Clear (N)" badge. |
| 2026-04-16 | **Inline row expansion in Transactions table** — clicking a row expands it in-place (blue highlight + rotating chevron). Expansion panel shows: full transaction details grid, category reassign select+save, exclude/restore button, "⚡ Turn into a rule" button (uses `extractWords` for meaningful keyword). `React.Fragment` with key replaces bare fragment. Category badge and exclude button stop propagation to prevent expand trigger. |
...
- [ ] Transactions page: "Net USD" column header should update to show reporting currency (currently always says "Net USD" even when ILS selected)
