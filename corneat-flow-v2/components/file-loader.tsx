...
import { parseWorkbook, parseSVBCsv, mergeTxns } from "@/lib/parsers";
...
// Non-consolidated entities (excludes "Consolidated")
...
export function FileLoader({ transactions, meta, serverOk, isAdmin, companies, onLoaded, onClear }: Props) {
  ...
  const [status, setStatus] = useState<{ added: number; dupes: number; total: number; errors: string[] } | null>(null);
  ...
  const processFiles = useCallback(async (files: File[]) => {
    ...
    for (const file of files) {
      ...
      if (!isXlsx && !isCsv) { errors.push(`${file.name}: unsupported format`); continue; }
      try {
        if (isXlsx) {
          const buf = await file.arrayBuffer();
          allNew = allNew.concat(parseWorkbook(new Uint8Array(buf), file.name));
        } else {
          ...
          if (firstLine.includes("From:") || text.split("\n")[1]?.includes("Bank ID")) {
            ...
            errors.push(`${file.name}: CSV format not recognised (expected SVB export)`);
          }
        }
      } catch (e) {
    ...
    // For admin uploading for a specific company, use that company's existing transactions for dedup
    ...
    // Override entity on all parsed transactions to match the selected company
    if (isAdmin && targetEntity) {
    ...
    <div className="border-b border-slate-200 bg-white px-5 py-3">
        ...
        >
          ...
          <div>
            ...
            <div className="mt-0.5 text-xs font-semibold">
              {serverOk
                ? <span className="text-green-600">✅ Connected to Supabase — data synced across all users</span>
      ...
      {status && (
        <div className={`mt-2 rounded-lg border px-3 py-2 text-xs ${
          status.errors.length ? "border-red-200 bg-red-50 text-red-700" : "border-green-200 bg-green-50 text-green-700"
        }`}>
          {status.added > 0 && <span>✅ {status.added} new transaction{status.added !== 1 ? "s" : ""} added{isAdmin ? ` to ${targetEntity}` : ""} · </span>}
          {status.dupes > 0 && <span>⏭ {status.dupes} duplicate{status.dupes !== 1 ? "s" : ""} skipped · </span>}
          <span>Total: {status.total}</span>
          {status.errors.map((e, i) => <div key={i} className="mt-1 text-red-600">⚠ {e}</div>)}
        </div>
      )}
    </div>
  );
