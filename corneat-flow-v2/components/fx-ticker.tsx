...
export function FxTicker({ onRates }: Props) {
  ...
  if (status === "loading") {
    return (
      <div className="flex items-center gap-1.5 rounded-md border border-white/10 bg-white/5 px-3 py-1">
        <span className="text-[10px] text-slate-500 animate-pulse">Loading FX…</span>
      </div>
    );
  }
  ...
  if (status === "error") {
    return (
      <div className="flex items-center gap-1.5 rounded-md border border-red-500/20 bg-red-500/10 px-3 py-1">
        <span className="text-[10px] text-red-400">FX unavailable</span>
      </div>
    );
  }
