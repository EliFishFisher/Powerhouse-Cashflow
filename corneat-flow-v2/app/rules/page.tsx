...
import { ALL_CATS, CAT_LABELS, CAT_COLORS, CAT_BG } from "@/lib/constants";
...
export default function RulesPage() {
  ...
  const openAdd = useCallback((prefill?: { keyword: string; cat: Category }) => {
    setEditId(null);
    setForm(prefill
      ? { label: prefill.keyword, keywords: prefill.keyword, field: "details" as RuleField, cat: prefill.cat, enabled: true }
      : EMPTY_FORM,
    );
    setPanelOpen(true);
  }, []);
    ...
    try {
      ...
      if (editId) {
        next = data.rules.map(r =>
          r.uid === editId
            ? { ...r, label: form.label.trim(), keywords, field: form.field, cat: form.cat, enabled: form.enabled }
            : r,
        );
      } else {
        next = [...data.rules, makeClassificationRule({
          label: form.label.trim(), keywords, field: form.field, cat: form.cat, enabled: form.enabled,
        })];
      }
